...
interface BlueBadgePageProps {
  ...
  language: Language;
  currency: Currency;
  onBuyBlueBadge: (plan: 'monthly' | 'yearly', balanceSource: 'deposit' | 'earning') => void;
  onOpenDeposit: () => void;
  onOpenKyc: () => void;
  onBack: () => void;
}
...
export const BlueBadgePage: React.FC<BlueBadgePageProps> = ({
  ...
  const handlePurchase = () => {
    ...
    setTimeout(() => {
      ...
      onBuyBlueBadge(selectedPlan, balanceSource);
      ...
      setSuccessMessage(
        isBn 
          ? `অভিনন্দন! আপনার ${selectedPlan === 'yearly' ? 'বাৎসরিক' : 'মাসিক'} ব্লু ভেরিফাইড ব্যাজ সক্রিয় হয়েছে।` 
    ...
    <div className="w-full max-w-4xl mx-auto space-y-6 pb-16 animate-in fade-in duration-200">
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
          ...
          <div className="flex-1 space-y-1.5">
            ...
            <p className="text-xs sm:text-sm text-blue-100 max-w-xl font-medium leading-relaxed">
              {isBn 
                ? 'আপনার নামের পাশে অফিসিয়াল ব্লু ব্যাজ যুক্ত করে ১০০% ট্রাস্ট অর্জন করুন। বায়ার ও ফ্রিল্যান্সারদের কাছে আপনার বিশ্বস্ততা বাড়বে দ্বিগুণ।' 
                : 'Build 100% trust with buyers and clients with an official blue checkmark badge on your profile and submissions.'}
            </p>
          </div>
        </div>
        ...
        {user.hasBlueBadge && (
          <div className="mt-6 pt-4 border-t border-white/20 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex items-center gap-2 text-emerald-300 font-bold">
              ...
              <span>
                {isBn ? 'আপনার একাউন্টে ব্লু ব্যাজ সক্রিয় আছে' : 'Blue Badge is currently active'}
              </span>
            </div>
            {user.blueBadgeExpiresAt && (
              <span className="text-blue-100 bg-white/10 px-3 py-1 rounded-full">
                {isBn ? 'মেয়াদ শেষ:' : 'Expires:'} {new Date(user.blueBadgeExpiresAt).toLocaleDateString()}
              </span>
            )}
          </div>
        )}
      ...
      <div className="flex items-center p-1.5 rounded-2xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
        ...
        >
          ...
          <span>{isBn ? 'তাৎক্ষণিক কিনুন (ওয়ালেট ব্যালেন্স)' : 'Buy Blue Badge (Wallet Balance)'}</span>
        </button>
      ...
      {/* TAB 1: BUY WITH WALLET BALANCE (DEPOSIT OR EARNING) */}
      {activeTab === 'buy' && (
        <div className="space-y-6">
          ...
          {/* STEP 3: Summary & Purchase Button */}
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
